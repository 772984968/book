<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:53:"/var/www/book/public/../app/admin/view/role/edit.html";i:1520738488;}*/ ?>
<!DOCTYPE html>
<html>
            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        <form class="form-horizontal" method="post" action="<?php echo url('role/edit'); ?>" id="form-admin-role-add">
                            <p>添加管理员角色(⊙o⊙)</p>
                            <div class="form-group">
                                <label class="col-sm-3 control-label">角色名称：</label>

                                <div class="col-sm-8">
                                    <input type="text" placeholder="请填写角色名称" class="form-control" name="name" value="<?php echo $data['name']; ?>">
                                    	<input type="hidden" class="input-text" value="<?php echo $data['id']; ?>" placeholder="" id="roleName" name="id">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-3 control-label">备注：</label>
                                <div class="col-sm-8">
                                    <input type="text" placeholder="角色描述" class="form-control" name="remark" value="<?php echo $data['remark']; ?>">
                                </div>
                            </div>
                               <div class="form-group">
                                <label class="col-sm-3 control-label">状态：</label>
                                <div class="col-sm-8">
                                <input type="radio" name="state" value="1" <?php if($data['state'] == 1): ?> checked="checked" <?php endif; ?>>正常<input type="radio" name="state" value="0"  <?php if($data['state'] == 0): ?> checked="checked" <?php endif; ?> >禁用
                                </div>
                            </div>
                            <div class="form-group">
...
<dl class="permission-list">
					<dt>
							...
							<input type="checkbox" value="<?php echo $vo['id']; ?>" name="access_ids[]" <?php if(in_array($vo['id'],$data['access_id'])): ?> checked="checked"<?php endif; ?>>
			...
			<?php if(is_array($vo['son']) || $vo['son'] instanceof \think\Collection || $vo['son'] instanceof \think\Paginator): $i = 0; $__LIST__ = $vo['son'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$voo): $mod = ($i % 2 );++$i;?>
					<dd>
						<dl class="cl permission-list2 dl-horizontal">
							<dt>
								<?php echo $voo['title']; ?>
									<input type="checkbox" value="<?php echo $voo['id']; ?>" name="access_ids[]" <?php if(in_array($voo['id'],$data['access_id'])): ?> checked="checked"<?php endif; ?> >
...
<?php if(is_array($voo['son']) || $voo['son'] instanceof \think\Collection || $voo['son'] instanceof \think\Paginator): $i = 0; $__LIST__ = $voo['son'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vooo): $mod = ($i % 2 );++$i;?>
									<?php echo $vooo['title']; ?>
									<input type="checkbox" value="<?php echo $vooo['id']; ?>" name="access_ids[]" <?php if(in_array($vooo['id'],$data['access_id'])): ?> checked="checked"<?php endif; ?>>
