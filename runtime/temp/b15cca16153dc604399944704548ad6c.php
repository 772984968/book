<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:59:"/var/www/book/public/../app/admin/view/./template/edit.html";i:1520738488;}*/ ?>
<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
...
<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
  <legend><?php echo $data['config']['edit']['title']; ?></legend>
</fieldset>
<form class="layui-form" action="<?php echo url($data['config']['edit']['url']); ?>" method="post">
				...
				<!-- password -->
				 <div class="layui-form-item">
				    ...
				    <div class="layui-input-inline">
				      <input type="password" name="<?php echo $vo['key']; ?>" lay-verify="" value="<?php echo $vo['value']; ?>" placeholder="请输入密码" autocomplete="off" class="layui-input">
				    </div>
				...
				<div class="layui-input-inline">
					<select name="<?php echo $vo['key']; ?>" lay-verify="">
					 <?php if(is_array($vo['option']) || $vo['option'] instanceof \think\Collection || $vo['option'] instanceof \think\Paginator): $i = 0; $__LIST__ = $vo['option'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$select): $mod = ($i % 2 );++$i;?>
						<option value="<?php echo $select['id']; ?>" <?php if($select['id'] == $vo['value']): ?>selected="selected"<?php endif; ?>><?php echo $select['name']; ?></option>
	...
	<?php case "radio": ?>
			<div class="layui-form-item">
				...
				<div class="layui-input-block">
					<?php if(is_array($vo['option']) || $vo['option'] instanceof \think\Collection || $vo['option'] instanceof \think\Paginator): $i = 0; $__LIST__ = $vo['option'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$radio): $mod = ($i % 2 );++$i;?>
					 <input type="radio"	name="<?php echo $vo['key']; ?>" value="<?php echo $radio['id']; ?>"

						title="<?php echo $radio['name']; ?>" <?php if($radio['id'] == $vo['value']): ?> checked="checked" <?php endif; ?>> <?php endforeach; endif; else: echo "" ;endif; ?>
				</div>
			</div>
			<?php break; ?>
<!-- date -->
...
<?php case "switch": ?>
 <div class="layui-form-item">
  ...
  <?php break; case "hidden": ?>
    ...
    <div class="layui-input-block">
      <input type="hidden" name="<?php echo $vo['key']; ?>" lay-verify=""  value="<?php echo $vo['value']; ?>"   <?php if(!(empty($vo['option']['placeholder']) || (($vo['option']['placeholder'] instanceof \think\Collection || $vo['option']['placeholder'] instanceof \think\Paginator ) && $vo['option']['placeholder']->isEmpty()))): ?>
